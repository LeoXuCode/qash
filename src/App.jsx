...
const IconBatches = ({color}) => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/>
  </svg>
);
const IconTools = ({color}) => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function BatchCard({b, t:tr, onExpand, expanded, onStartSell, sellingId, sellPrices, onSellPriceChange, onConfirmSell, onCancelSell, onDelete, confirmDelete, onConfirmDelete, onCancelDelete, onStartEdit, editingBatchId, batchForm, setBatchForm, submitBatch, cancelEditBatch, updateItem, addItem, removeItem, defaultCur, isSelecting, selectedBatchIds, onToggleSelect, categories}) {
  ...
  return (
    <div style={{background:"rgba(255,255,255,0.05)",borderRadius:24,padding:"18px",border:"1px solid rgba(255,255,255,0.08)",marginBottom:10,cursor:isSelecting?"pointer":"pointer",display:"flex",flexDirection:"column",alignItems:"flex-start",gap:isSelecting?12:0}}
      onClick={() => isSelecting ? onToggleSelect(b.id) : onExpand(b.id)}>
      {isSelecting && (
        <div style={{width:22,height:22,borderRadius:"50%",border:"2px solid "+(isSelected?"#4ade80":"rgba(255,255,255,0.3)"),background:isSelected?"#4ade80":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:2,position:"absolute",left:18,top:18}}>
          {isSelected && <span style={{color:"#000",fontSize:14,fontWeight:700,lineHeight:1}}>✓</span>}
        </div>
      )}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flex:1,width:"100%",paddingLeft:isSelecting?44:0}}>
        <div style={{flex:1}}>
      ...
      {isExp && (
        <div style={{marginTop:18,paddingTop:16,borderTop:"1px solid rgba(255,255,255,0.06)",width:"100%"}} onClick={e => e.stopPropagation()}>
          {isEditing && (
            <div style={{padding:"16px",marginBottom:16,borderRadius:12,background:"rgba(96,165,250,0.06)",border:"1px solid rgba(96,165,250,0.15)",width:"100%",boxSizing:"border-box"}}>
              <input placeholder={tr.batchName} value={batchForm.name} onChange={e => setBatchForm(f => ({...f,name:e.target.value}))} style={{...iStyle,marginBottom:12,fontSize:15}}/>
              ...
              <div style={{marginBottom:12}}>
                ...
                <select value={batchForm.category} onChange={e => setBatchForm(f => ({...f,category:e.target.value}))} style={{...iStyle,fontSize:15}}>
                  ...
                  {categories && categories.map(c => <option key={c} value={c} style={{background:"#1a1a1e"}}>{c}</option>)}
                </select>
              </div>
              <div style={{marginBottom:14}}>
                <div style={{fontSize:12,fontWeight:600,marginBottom:10,color:"#999",textTransform:"uppercase",letterSpacing:1}}>Items</div>
                {batchForm.items.map((it,idx) => (
                  <div key={it.id} style={{padding:"12px",marginBottom:8,borderRadius:12,background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.05)"}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                      <div style={{fontSize:11,color:"#666",fontWeight:600}}>Item {idx+1}</div>
                    ...
                    <div style={{display:"flex",gap:6,marginBottom:6}}>
                      ...
                      <div style={{fontSize:9,color:"#666",textTransform:"uppercase",letterSpacing:1,paddingTop:2}}>Qty</div>
                    ...
                    <div style={{display:"flex",gap:6,marginBottom:6}}>
                      ...
                      <select value={it.buyCur||"NOK"} onChange={e => updateItem(idx,"buyCur",e.target.value)} style={{...iStyle,flex:1,fontSize:13,padding:"10px 8px"}}>
                        {Object.keys(RATES).map(c => <option key={c} value={c} style={{background:"#1a1a1e"}}>{c}</option>)}
                      </select>
                    </div>
                    {batchForm.status==="sold" && (
                      <div style={{display:"flex",gap:6}}>
                        ...
                        <select value={it.sellCur||"NOK"} onChange={e => updateItem(idx,"sellCur",e.target.value)} style={{...iStyle,flex:1,fontSize:13,padding:"10px 8px"}}>
                          {Object.keys(RATES).map(c => <option key={c} value={c} style={{background:"#1a1a1e"}}>{c}</option>)}
                        </select>
                      </div>
                    )}
              ...
              <button onClick={addItem} style={{width:"100%",padding:"10px",border:"1px dashed rgba(255,255,255,0.12)",borderRadius:10,background:"none",color:"#888",fontSize:12,cursor:"pointer",fontFamily:"inherit",fontWeight:500,marginBottom:14}}>{tr.addItem}</button>
              <div style={{display:"flex",gap:8}}>
                <button onClick={cancelEditBatch} style={{flex:1,padding:"13px 16px",border:"1px solid rgba(255,255,255,0.1)",borderRadius:12,background:"none",color:"#999",fontSize:14,cursor:"pointer",fontFamily:"inherit",fontWeight:600}}>{tr.cancel}</button>
                <button onClick={submitBatch} style={{flex:1,padding:"13px 16px",border:"none",borderRadius:12,background:"#4ade80",color:"#000",fontSize:14,cursor:"pointer",fontFamily:"inherit",fontWeight:700}}>Save</button>
              </div>
            </div>
          )}
          {isSelling && (
...
export default function App() {
  ...
  const [activeCatFilter, setActiveCatFilter] = useState(() => {
    try { return JSON.parse(localStorage.getItem("qash_activeCatFilter")||"null"); } catch(e) { return null; }
  });
  ...
  useEffect(() => { try { localStorage.setItem("qash_activeCatFilter", JSON.stringify(activeCatFilter)); } catch(e) {} }, [activeCatFilter]);
  ...
  const startEditBatch = (b) => {
    ...
    const processedItems = b.items.map(it => ({
      ...it,
      id: it.id || Date.now() + Math.random(),
      buyCur: it.buyCur || "NOK",
      sellCur: it.sellCur || "NOK",
      qty: it.qty || 1,
      buyCost: it.buyCost || "",
  ...
  const batchProps = {
    ...
    isSelecting, selectedBatchIds, onToggleSelect:toggleSelectBatch, categories,
  };
  ...
  const TABS = [
    {name:tr.home, component:IconHome},
    {name:"Batches", component:IconBatches},
    {name:tr.calculator, component:IconTools},
    {name:tr.forecast, component:IconForecast},
    {name:tr.settings, component:IconSettings}
  ];
      ...
      <div style={{padding:"16px 20px"}}>
        <div>
          ...
          <div style={{fontSize:13,color:"#666",marginTop:2,display:"flex",alignItems:"center",gap:6}}>
            ...
            {ratesLoaded && <span style={{fontSize:13,color:"#4ade80",lineHeight:1,marginTop:"-2px"}}>●</span>}
          </div>
        </div>
      </div>
